...
exports.unitExport = exports.abilityExport = void 0;
...
const cmdExcel2KV_1 = require("./cmdExcel2KV");
async function abilityExport(context, uri) {
    // 当前文件路径
    ...
    if (abilityExcelConfig) {
        (0, cmdExcel2KV_1.eachExcelConfig)(abilityExcelConfig, async (kvDir, excelDir) => {
            let fileType = (await vscode.workspace.fs.stat(vscode.Uri.file(kvDir))).type;
    ...
    async function keyValue2CSV(kvPath, csvPath) {
        // let csv_path = path.dirname(excel_object[index]);
        if (await (0, pathUtils_1.getPathInfo)(csvPath) === false) {
            // util.ShowError("不存在csv文件");
            return;
        }
        let csv = (0, csvUtils_1.csv2array)(fs.readFileSync(csvPath, 'utf-8'));
        let kv = (0, kvUtils_1.readKeyValue2)(fs.readFileSync(kvPath, 'utf-8'));
        let csvTitle = csv[0];
        let csvKey = csv[1];
        ...
        for (const abilityName in kv[Object.keys(kv)[0]]) {
            ...
            for (const abilityKey in abilityData) {
                ...
                if (abilityKey === 'AbilitySpecial') { //特殊处理AbilitySpecial
                    let specialCount = 1; //记录第几个special值
        ...
        fs.writeFileSync(csvPath, (0, csvUtils_1.array2csv)(finalCsv));
    }
}
exports.abilityExport = abilityExport;
async function unitExport(context, uri) {
    // 当前文件路径
    let filePath = path.normalize(uri.fsPath);
    let unitExcelConfig = vscode.workspace.getConfiguration().get('dota2-tools.A4.UnitExcel');
    if (unitExcelConfig) {
        (0, cmdExcel2KV_1.eachExcelConfig)(unitExcelConfig, async (kvDir, excelDir) => {
            let fileType = (await vscode.workspace.fs.stat(vscode.Uri.file(kvDir))).type;
            if (fileType === vscode.FileType.Directory) {
                if (filePath.indexOf(kvDir) !== -1) {
                    let csvPath = path.join(excelDir, 'csv', path.basename(filePath).replace(path.extname(filePath), '.csv'));
        ...
        for (const unitName in kv[Object.keys(kv)[0]]) {
            ...
            for (const unitKey in unitData) {
                ...
                if (unitKey === 'Creature') { //特殊处理AttachWearables
                    let wearableCount = 1; //记录第几个AttachWearables值
...
exports.unitExport = unitExport;
//# sourceMappingURL=cmdKvExport.js.map