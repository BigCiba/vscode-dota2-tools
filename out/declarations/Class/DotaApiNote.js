...
exports.DotaApiNote = void 0;
const fs = require("fs");
const mysql = require("mysql2");
const path = require("path");
const apiParse_1 = require("../../utils/apiParse");
/**
 * Dota2接口笔记相关的类
 */
class DotaApiNote {
    constructor(context) {
        ...
        [this.classList, this.enumList] = (0, apiParse_1.apiParse)(this.context, this.api_note);
    }
    // 初始化获取数据
    init(callback) {
        let initCount = 0;
    ...
    // 数据库数据转function
    formatLuaFunction(funcInfo) {
        let res = {};
        if (funcInfo.class) {
            res.class = funcInfo.class;
        }
        if (funcInfo.class_cl) {
            res.class_cl = funcInfo.class_cl;
        }
        if (funcInfo.type) {
            res.type = funcInfo.type;
        }
        if (funcInfo.function) {
            res.function = funcInfo.function;
        }
        if (funcInfo.return) {
            res.return = funcInfo.return;
        }
        if (funcInfo.description) {
            res.description = funcInfo.description;
        }
        if (funcInfo.server) {
            res.server = (funcInfo.server == 1 ? true : false);
        }
        if (funcInfo.client) {
            res.client = (funcInfo.client == 1 ? true : false);
        }
        if (funcInfo.example) {
            res.example = funcInfo.example;
        }
        if (funcInfo.params) {
    ...
    // 数据库数据转enum
    formatLuaEnum(enumInfo) {
        let res = {};
        if (enumInfo.class) {
            res.class = enumInfo.class;
        }
        if (enumInfo.type) {
            res.type = enumInfo.type;
        }
        if (enumInfo.name) {
            res.name = enumInfo.name;
        }
        if (enumInfo.value != undefined) {
            res.value = enumInfo.value;
        }
        if (enumInfo.function) {
            res.function = enumInfo.function;
        }
        if (enumInfo.description) {
            res.description = enumInfo.description;
        }
        if (enumInfo.description_lite) {
            res.description_lite = enumInfo.description_lite;
        }
        if (enumInfo.example) {
            res.example = enumInfo.example;
        }
        return res;
    }
    // 数据库数据转css
    formatCssProperty(cssProperty) {
        let res = {};
        if (cssProperty.class) {
            res.class = cssProperty.class;
        }
        if (cssProperty.value != undefined && cssProperty.value != "") {
            res.value = JSON.parse(cssProperty.value);
        }
        if (cssProperty.description) {
            res.description = cssProperty.description;
        }
        if (cssProperty.example) {
            res.example = cssProperty.example;
        }
        return res;
    }
    // 获取注释数据
    ...
    // 从数据库获取函数的注释
    getFunctionNote(funcInfo, callback) {
...
exports.DotaApiNote = DotaApiNote;
//# sourceMappingURL=DotaApiNote.js.map